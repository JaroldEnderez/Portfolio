...
export default function ProjectsPage({params}) {
  ...
  return (
      <main className="pt-16 flex flex-col items-center justify-center w-full pb-20 p-4">
        ...
        viewport={{ once: true }}
        >
          <img src={project.image.src} alt={project.title} className="rounded-lg my-4 w-fullmax-w-[600px]" />
          <div className="flex flex-col sm:flex-row gap-4 p-4 items-center">
            <div className="flex border px-4 py-2 rounded-full items-center justify-center">
              <h1 className="text-2xl sm:text-3xl text-white">{project.title}</h1>
            </div>  
            <div className="flex border px-4 py-2 rounded-full items-center justify-center">
              <h1 className="text-2xl sm:text-3xl text-white">2024</h1>
            </div>
            <a 
              href={project.link}     
              target="_blank"   
              rel="noopener noreferrer"
              className="text-white bg-blue-600 px-4 py-2 rounded-lg text-sm sm:text-base"
            >
              Live demo →
            </a>
        </div>
        ...
        >
          <div className='w-full max-w-300 mt-5'>
            ...
            <p className='text-justify text-2xl sm:text-'>
              {project.process}
            </p>
          </div>

          <div className='flex justify-between w-full max-w-300 sm:flex-wrap flex-col'>
